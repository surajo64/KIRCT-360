import mongoose from "mongoose";

const adminSchema = new mongoose.Schema(
  {
    name: { type: String, required: true },
    email: { type: String, required: true, unique: true },
    phone: { type: String, required: true },
    password: { type: String, required: true },
    about: { type: String, default: "" },
    role: { type: String, enum: ["admin", "educator"], required: true },
    image: { type: String, default: "data:image/png;base64," },
    address: { type: Object, default: 'not selected' },
    gender: { type: String, default: "not selected" },
    nin: { type: String, default: "not selected" },
    dob: { type: Date, default: "1990/01/01" },
    isActive: { type: Boolean, default: true },
    courses: [
      {
        type: mongoose.Schema.Types.ObjectId,
        ref: 'Course'
      }
    ],
    resetPasswordToken: { type: String, default: null },
    resetPasswordExpires: { type: Date, default: null },
  },
  { timestamps: true } // Auto-add createdAt & updatedAt
);

// Pre-save hook to enforce the rule
adminSchema.pre("save", function (next) {
  if (this.role !== "educator") {
    this.courses = undefined; // strip out courses for non-educators
  }
  next();
});

const Admin = mongoose.model("Admin", adminSchema);

export default Admin;
